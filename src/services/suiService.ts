...
export class SuiMailService {
  ...
  async addMembersAndSendMail(
    ...
    // 1. Add sender as member using address-only function (no profile required)
    tx.moveCall({
    ...
    // 2. Add each recipient as member using address-only function
    for (const recipient of recipients) {
      // Skip if recipient is same as sender (already added above)
      if (recipient === senderAddress) continue;
  ...
  async getReceivedMails(userAddress: string) {
    ...
    for (const allowlistId of allowlistIds) {
      ...
      const receivedMails = mails.filter(mail =>
        mail.data?.content?.fields?.sender !== userAddress
      );
