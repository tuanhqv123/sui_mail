...
import { ArrowLeft, Loader2, FileText, Download, Copy, ChevronDown, ChevronUp } from "lucide-react";
...
const Sent = () => {
     ...
     const decryptAndLoadMail = async (email: SentMail): Promise<SentMail> => {
          ...
          try {
               ...
               setSentMails((prevMails) => prevMails.map((m) => (m.id === email.id ? updatedEmail : m)));
     ...
     const backgroundLoadTimestamps = async (mailList: SentMail[]) => {
          ...
          for (const mail of mailList) {
               if (mail.isEncrypted && mail.allowlistId) {
                    try {
                         const mailContent = await decryptionService.decryptMail(mail.blobId, mail.allowlistId);
                         ...
                         if (mailContent.timestamp) {
                              ...
                              setSentMails((prevMails) =>
                                   prevMails.map((m) => (m.id === mail.id ? { ...m, time: timestamp } : m)),
                              );
     ...
     const loadSentMails = async () => {
          ...
          try {
               const mailObjects = await suiMailService.getSentMails(currentAccount.address);
                                        ...
                                        {selectedEmail.attachments.map((attachment: any, index: number) => (
                                             ...
                                             >
                                                  ...
                                                  <div>
                                                       ...
                                                       <p className="text-xs text-black/50">{attachment.size} bytes</p>
                                                  </div>
                                                  <button
     ...
     return (
          <div className="p-8 h-full">
               ...
               {sentMails.length === 0 ? (
                    <div className="text-center py-20 text-gray-500">No sent messages yet.</div>
               ) : (
                    <div className="space-y-2">
                         {sentMails.map((mail) => (
                              <div
                              ...
                              >
                                   <div className="flex justify-between items-start mb-2">
                                        <div className="flex-1">
                                             <h3 className="text-black font-semibold text-base">{mail.subject}</h3>
                                             {mail.sender && (
                                   ...
                                   <p className="text-sm text-gray-500 line-clamp-2">{mail.preview}</p>
                              </div>
                         ))}
                    </div>
               )}
