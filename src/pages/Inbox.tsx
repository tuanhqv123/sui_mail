...
import { ArrowLeft, Reply, FileText, Download, Loader2, Copy, Users, ChevronDown, ChevronUp } from "lucide-react";
...
import { formatFileSize, type MailContent, type AttachmentInfo } from "../utils/encryption";
...
const Inbox = () => {
     ...
     const [recipientNames, setRecipientNames] = useState<Map<string, string | null>>(new Map());
     ...
     const suiMailService = useMemo(() => new SuiMailService(suiClient), [suiClient]);
     const walrusService = useMemo(() => new WalrusService(), []);
     const decryptionService = useMemo(() => MailDecryptionService.getInstance(suiClient), [suiClient]);
     ...
     const decryptAndLoadMail = async (email: Email): Promise<Email> => {
          ...
          try {
               ...
               if (email.isEncrypted && email.allowlistId && sessionInitialized) {
                    try {
                         mailContent = await decryptionService.decryptMail(email.blobId, email.allowlistId);
               ...
               const timestamp = mailContent.timestamp ? formatTimestamp(mailContent.timestamp) : email.time;
                    ...
                    try {
                         ...
                         if (sessionInitialized && fields.allowlist_id) {
                              try {
                                   ...
                                   mailContent = await walrusService.downloadBlobAsJson(fields.blob_id);
                              }
                         } else {
                              mailContent = await walrusService.downloadBlobAsJson(fields.blob_id);
                         }
                         ...
                         const timestamp = new Date(
                              typeof mailTimestamp === "string" ? mailTimestamp : parseInt(mailTimestamp),
                         ).toLocaleString();
     ...
     const loadEmails = async () => {
          ...
          try {
               const mailObjects = await suiMailService.getReceivedMails(currentAccount.address);
               ...
               const mailThreading = await suiMailService.getMailThreadingForUser(currentAccount.address);
     ...
     const backgroundLoadTimestamps = async (emailList: Email[]) => {
          ...
          for (const email of emailList) {
               if (email.isEncrypted && email.allowlistId) {
                    try {
                         const mailContent = await decryptionService.decryptMail(email.blobId, email.allowlistId);
                         ...
                         if (mailContent.timestamp) {
                              ...
                              setEmails((prevEmails) =>
                                   prevEmails.map((e) => (e.id === email.id ? { ...e, time: timestamp } : e)),
                              );
                    ...
                    <div className="flex-1 overflow-y-auto">
                         ...
                         {selectedEmail.attachments && selectedEmail.attachments.length > 0 && (
                              <div className="mb-8">
                                   ...
                                   <div className="space-y-2">
                                        {selectedEmail.attachments.map((attachment: AttachmentInfo, index: number) => (
                                             <div
                                                  key={index}
                         ...
                         {loadingParent && (
                              <div className="flex items-center justify-center py-8">
                                   ...
                                   <span className="text-sm text-gray-500">Loading parent mail...</span>
                              </div>
                         )}
                              ...
                              <div className="border-t border-gray-200 pt-6 mt-6">
                                   <div className="mb-4">
                                        ...
                                        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                                             ...
                                             {parentMail.attachments && parentMail.attachments.length > 0 && (
     ...
     return (
          <div className="p-8 h-full">
               <header className="flex justify-between items-center mb-8">
                    ...
                    <div className="flex items-center gap-4">
                         {initializingSession && (
                              <span className="text-sm text-blue-600 font-medium">Initializing decryption...</span>
                         )}
                         {sessionInitialized && (
                              <span className="text-sm text-green-600 font-medium">Decryption Active</span>
                         )}
                    </div>
               </header>

               {emails.length === 0 ? (
                    <div className="text-center py-20 text-gray-500">No messages in your inbox yet.</div>
               ) : (
                    <div className="space-y-2">
                         {emails.map((email) => (
                              <div
